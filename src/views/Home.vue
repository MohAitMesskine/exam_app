<template>
  ...
  <QuizCompleteOverlay v-if="endOfQuiz" :percent="percentageScore" @restartQuiz="onQuizStart"></QuizCompleteOverlay>
    ...
    const onOptionClicked = (choice, item) => {
      if (canClick) {
        ...
        if (currentQuestion.value.answer == optionID) {
          ...
          score.value += 10;
          divContainer.classList.add("bg-green-500");
          divContainer.classList.remove("option-default");
        } else {
          console.log("you are wrong");
          divContainer.classList.add("bg-red-500");
          divContainer.classList.remove("option-default");
        }
        timer.value = 100;
        canClick = false;
